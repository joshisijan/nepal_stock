...
class NepalStockScrap {
  ...
  static Future<MarketInfo> getMarketInfo() async {
    List<Map<String, dynamic>> elements, info;
    MarketInfo marketInfo;
    if (await webScraper.loadWebPage('')) {
      elements = webScraper
          .getElement('#market-watch .panel-body table tbody tr td', []);
      info = webScraper
          .getElement('#market-watch .panel-body .top_marketinfo', []);
    }
    marketInfo = MarketInfo();
  ...
  static symbolChangeDetail(String n) async {
    List<Map<String, dynamic>> elements;
    if (await webScraper.loadWebPage('')) {
      elements = webScraper.getElement('#stockData marquee', []);
    }
    return elements;
  }
}
